...
(define (hidden-token? tok)
  ...
  (let ([t (position-token-token tok)]
        [hidden-tokens
         (map
          (lambda (tp) (token-name (tp '())))
          (list token-WS
                token-COMMENT
                token-LINE_COMMENT))])
    (and (token? t)
         (memq t hidden-tokens)
         #t)))
(define (tokenize-java file)
  (let ([input (open-input-file file)])
    (let loop ([tokens '()])
      (let ([tok (java-lexer input)])
        (if (eq? (position-token-token tok) 'EOF)
            (reverse tokens)
            (loop (if (hidden-token? tok)
                      tokens
                      (cons tok tokens))))))))
...
(define (token-name* tok)
  (let ([t (position-token-token tok)])
    (if (token? t)
        (token-name t)
        tok)))
; (token-name* (token-ADD))
(token-name* (token-WS '()))
(module+ test
  (pretty-print (tokenize-java "example/Simple.java")))